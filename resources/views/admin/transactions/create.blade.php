...
<head>
    ...
    <title>Input Transaksi Baru - Catering Nur Baluwarti</title>
    ...
    <style>
        ...
        /* Memaksa ikon date & time picker browser berwarna emas */
...
<body class="bg-luxury-dark min-h-screen text-slate-100 flex flex-col">
    ...
    <main class="flex-grow p-4 md:p-8 max-w-[850px] w-full mx-auto space-y-6">
        ...
        <div class="flex items-center justify-between border-b border-white/5 pb-4">
            <div class="space-y-0.5">
                <h2 class="font-serif text-xl md:text-2xl font-bold text-white tracking-wide">Pencatatan Transaksi Manual</h2>
                <p class="text-xs text-slate-400">Catat invoice masuk dari pesanan offline/WhatsApp ke dalam sistem database riel.</p>
            </div>
        ...
        <div class="luxury-glass rounded-2xl p-6 md:p-8 border border-white/5 shadow-2xl">
            ...
            <form method="POST" action="{{ url('/admin/transactions') }}" onsubmit="singkronkanNamaCustomer()">
                @csrf

                <div class="space-y-2 mb-6">
                    <label class="block text-[10px] font-bold uppercase tracking-widest text-slate-400">Kode Invoice / Order</label>
                    <input type="text" name="order_code" value="INV-{{ strtoupper(Str::random(6)) }}" required readonly
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div class="space-y-2">
                        ...
                        <input id="nama_pemesan" type="text" name="nama_pemesan" value="{{ old('nama_pemesan') }}" placeholder="Contoh: Ibu Rahayu Surakarta" required
                    ...
                    <div class="space-y-2">
                        ...
                        <input id="nomor_wa" type="tel" name="nomor_wa" value="{{ old('nomor_wa') }}" placeholder="Contoh: 08123456789" required
                               class="luxury-input w-full px-4 py-2.5 bg-black/30 border border-white/10 rounded-xl text-sm text-white placeholder-slate-600 focus:outline-none">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div class="space-y-2">
                        ...
                        <select id="package_name" name="package_name" required onchange="hitungTotalManual()" class="luxury-input w-full px-4 py-2.5 bg-black/30 border border-white/10 rounded-xl text-sm text-white focus:outline-none">
                            <option value="" disabled selected class="bg-slate-950 text-slate-500">-- Pilih Paket Hidangan --</option>
                            @foreach(($packages ?? []) as $pkg)
                                <option value="{{ $pkg->package_name }}" data-price="{{ $pkg->price }}" class="bg-slate-950 text-slate-200">
                    ...
                    <div class="space-y-2">
                        ...
                        <input id="jumlah_porsi" type="number" name="jumlah_porsi" min="1" value="{{ old('jumlah_porsi', 1) }}" required oninput="hitungTotalManual()" placeholder="Ketik jumlah porsi..."
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div class="space-y-2">
                        ...
                        <input id="tanggal_acara" type="date" name="tanggal_acara" value="{{ old('tanggal_acara', date('Y-m-d')) }}" required
                    ...
                    <div class="space-y-2">
                        <label for="delivery_time" class="block text-[10px] font-bold uppercase tracking-widest text-slate-400">Waktu Pengiriman (Jam Siap Kirim)</label>
                        <input id="delivery_time" type="time" name="delivery_time" value="{{ old('delivery_time', '11:00') }}" required
                               class="luxury-input w-full px-4 py-2.5 bg-black/30 border border-white/10 rounded-xl text-sm font-mono text-white focus:outline-none">
                    </div>
                </div>
                ...
                <div class="space-y-2 mb-6">
                    ...
                    <input id="tempat_antar" type="text" name="tempat_antar" value="{{ old('tempat_antar') }}" placeholder="Contoh: Gedung Aula Graha Soloraya, Jl. Slamet Riyadi No. 10" required
                           class="luxury-input w-full px-4 py-2.5 bg-black/30 border border-white/10 rounded-xl text-sm text-white placeholder-slate-600 focus:outline-none">
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div class="space-y-2">
                        ...
                        <select id="status" name="status" class="luxury-input w-full px-4 py-2.5 bg-black/30 border border-white/10 rounded-xl text-sm text-white focus:outline-none">
                            <option value="Sukses" class="bg-slate-950">Sukses / Settlement (Lunas)</option>
                            <option value="Pending" class="bg-slate-950">Pending / Menunggu Pembayaran COD</option>
                        </select>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="w-full px-4 py-2.5 bg-emerald-500/5 border border-emerald-500/20 rounded-xl text-sm font-mono font-bold text-emerald-400 flex items-center h-[45px]">
                            <span id="total_bayar_display">Rp 0</span>
                        </div>
                ...
                <div class="flex justify-end gap-3 pt-2 border-t border-white/5">
                    <button type="submit" class="px-6 py-3 bg-[#c29b40] hover:bg-[#b08a35] text-slate-950 text-xs font-bold uppercase tracking-widest rounded-xl shadow-lg transition duration-200">
                        <i class="fa-solid fa-file-invoice-dollar mr-1"></i> Simpan Catatan Invoice
                    </button>
                </div>
            </form>
        </div>
    ...
    <script type="text/javascript">
        function hitungTotalManual() {
            ...
            // Mengambil nominal harga dari atribut data-price di option terpilih
            var hargaPerPorsi = 0;
            ...
            // Rumus kalkulator finansial
            var totalBayar = hargaPerPorsi * volumePorsi;
            
            // Masukkan nominal mentah ke hidden input untuk dikirim ke database
            document.getElementById("total_bayar_hidden").value = totalBayar;
            
            // Format nominal menjadi rupiah untuk interface admin
            document.getElementById("total_bayar_display").innerText = "Rp " + totalBayar.toLocaleString('id-ID');
        }

        function singkronkanNamaCustomer() {
            // Memastikan customer_name terisi string yang bersih dari spasi gaib ujung
            var namaPemesan = document.getElementById("nama_pemesan").value;
            document.getElementById("customer_name").value = namaPemesan.trim();
        }
    </script>
</body>
</html>